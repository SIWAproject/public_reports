...
Row {data-height=70}
-----------------------------------------------------------------------
When analyzing histopathology results, the specific trait of inflammation severity in the ileum stood out. Anova test indicates inflammation severity tends to change with treatment use in ileum (p-value=0.03). Specifically there is a significant change between samples of MediumLowCa+VitD and samples of MediumLowCa, the last one having lower Inflammation scores, which could indicate that the addition of VitD using the recommended level of calcium in diet could reduce the damages in the architecture or integrity of the intestines related with inflammation. (Fig 28)
...
Row {data-height=80}
-----------------------------------------------------------------------
The SIWA ratios seek to broadly explain the composition of the microbiome in a single indicator. thus their behavior across treatments and across vitamin D levels is of interest. Some significant differences were found, nevertheless no trend was constant, specially with the use of VitD product, which means that the AlphaD3 product does not greatly modulate gut microbiom (Fig 31 and 32).
...
Row {data-height=70}
-----------------------------------------------------------------------
Benchmarks help compare a group of samples to a larger population of animals that have the same age and wide range of diets, in that way one can understand if the group of interest falls into the average range or if it is an outlier group in any microbiome indicator. In this case we evaluated how the different calcium level fall into a larger population of 42 days old broilers in terms of SIWA ratio 1 (Lactobacillus/ Escherichia-Shigella) value (Fig 33). As well as with and without vitamin D with the recomended level of calclium (Fig 34). In none of the benchmarks the groups of interest stood out of the average range for SIWA Ratio 1. The same was found using alpha divesity of the microbiome as an indicator (not shown).
...
Row {data-height=100}
-----------------------------------------------------------------------
In addition, a partition of samples was made not by their diet but by their performance and it was analyzed if there was any differentially abundant bacteria between the samples that had a good performance (lower FCR) vs. the samples that had low performance (higher FCR) (Fig 36). Indeed there are some bacterial groups that differ between these conditions of the animal, however, they do not match the groups that are increased when using vitamin D in the diet. Therefore it is not possible to affirm that the vitamin D increases bacteria that are beneficial for the animal or are related to an increase in productivity.
