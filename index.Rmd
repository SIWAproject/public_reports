...
```{r All inputs MARIA, include=FALSE}
...
kbl(ttab, centering = FALSE) %>% 
  kable_styling(full_width = F, position = "center") #agregando lo siguiente podria cambiar el color de los numeros en las columnas 2 y 3 por ejemplo %>%
...
#####Plot Taxonomic composition cecum#####  
#Aggregate OTUs according to genus
ODLEPobj_cecum_rel <- microbiome::transform(ODLEPobj_cecum, "compositional")
grouped <- aggregate_taxa_siwa(ODLEPobj_cecum_rel, "Genus")
melted <- psmelt(otu_table(grouped,taxa_are_rows=TRUE))

#create genus label for ploting, keeping only top x
ps.com.gen <- microbiomeutilities::aggregate_top_taxa2(ODLEPobj_cecum, top = 12, "Genus")
keep_genus <-as.list(as.data.frame(tax_table(ps.com.gen))$Genus)
keep_genus <- keep_genus[keep_genus!= "UNKNOWN"]
keep_genus <- keep_genus[keep_genus!= "Other"]
melted$label <- as.character(if_else(melted$OTU %in% keep_genus, melted$OTU, "Others"))

#add treatment info 
melted$SampleID <- (melted$Sample)
metadata_cec <- sample_data(ODLEPobj_cecum)
melted <- dplyr::left_join(melted, metadata_cec, by="SampleID")
melted <- melted %>%
...
#Figures abundances
p <- ggplot(data=melted, aes(x=Treatment, y=Abundance ,fill=label))
plot_abundance_cecum<- p + geom_bar(stat="identity", position="fill") + 
  labs(title ="Taxonomic composition cecum", fill="Genera") +
  xlab("Treatment") +
...
#Aggregate OTUs according to genus
ODLEPobj_ileum_rel <- microbiome::transform(ODLEPobj_ileum, "compositional")
grouped <- aggregate_taxa_siwa(ODLEPobj_ileum_rel, "Genus")
melted <- psmelt(otu_table(grouped,taxa_are_rows=TRUE))

#create genus label for ploting, keeping only top x
ps.com.gen <- microbiomeutilities::aggregate_top_taxa2(ODLEPobj_ileum, top = 12, "Genus")
keep_genus <-as.list(as.data.frame(tax_table(ps.com.gen))$Genus)
keep_genus <- keep_genus[keep_genus!= "UNKNOWN"]
keep_genus <- keep_genus[keep_genus!= "Other"]
melted$label <- as.character(if_else(melted$OTU %in% keep_genus, melted$OTU, "Others"))
...
#add treatment info 
melted$SampleID <- (melted$Sample)
metadata_ile <- sample_data(ODLEPobj_ileum)
melted <- dplyr::left_join(melted, metadata_ile, by="SampleID")
melted <- melted %>%
  mutate( Treatment=factor(Treatment,levels=c("LowCa", "LowCa+VitD", "MediumLowCa", "MediumLowCa+VitD", "MediumHighCa", "MediumHighCa+VitD", "HighCa","HighCa+VitD")) )
...
plot_abundance_ileum<- p + geom_bar(stat="identity", position="fill") + 
  labs(title ="Taxonomic composition ileum", fill="Genera") +
  xlab("Treatment") +
  ylab("Relative abundance")+
  #stat_compare_means(method = "anova")+ 
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
...
#Aggregate OTUs according to genus
ODLEPobj_feces_rel <- microbiome::transform(ODLEPobj_feces, "compositional")
grouped <- aggregate_taxa_siwa(ODLEPobj_feces_rel, "Genus")
melted <- psmelt(otu_table(grouped,taxa_are_rows=TRUE))

#create genus label for ploting, keeping only top x
ps.com.gen <- microbiomeutilities::aggregate_top_taxa2(ODLEPobj_feces, top = 12, "Genus")
keep_genus <-as.list(as.data.frame(tax_table(ps.com.gen))$Genus)
keep_genus <- keep_genus[keep_genus!= "UNKNOWN"]
keep_genus <- keep_genus[keep_genus!= "Other"]
melted$label <- as.character(if_else(melted$OTU %in% keep_genus, melted$OTU, "Others"))
...
#add treatment info 
melted$SampleID <- (melted$Sample)
metadata_fec <- sample_data(ODLEPobj_feces)
melted <- dplyr::left_join(melted, metadata_fec, by="SampleID")
melted <- melted %>%
  mutate( Treatment=factor(Treatment,levels=c("LowCa", "LowCa+VitD", "MediumLowCa", "MediumLowCa+VitD", "MediumHighCa", "MediumHighCa+VitD", "HighCa","HighCa+VitD")) )
...
plot_abundance_feces<- p + geom_bar(stat="identity", position="fill") + 
  labs(title ="Taxonomic composition feces", fill="Genera") +
  xlab("Treatment") +
  ylab("Relative abundance")+
  #stat_compare_means(method = "anova")+ 
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
...
# Extra graphs  {data-icon="fa fa-bookmark-o"}
Row {data-height=50}
-----------------------------------------------------------------------

Row {data-height=30}
-----------------------------------------------------------------------
**Additional visualizations of data**

Row {data-height=50}
-----------------------------------------------------------------------
Extra analysis added after discussion of initial results with the project owner.

```{r, include=FALSE}
#add treatment and kit id
complete_sample_table <- complete_sample_table %>% 
  mutate(Treatment = if_else(KitID == 70, "LowCa", 
                             ifelse(KitID == 71, "LowCa+VitD",
                                    ifelse(KitID == 72, "MediumLowCa",
                                           ifelse(KitID == 73, "MediumLowCa+VitD",
...
Row {data-height=30}
-----------------------------------------------------------------------
**Performance: FCR across treatments **

Row {data-height=70}
-----------------------------------------------------------------------
Feed conversion rate (from day 22 to day 33) changes with treatments, specifically there is a trend for it to increase with the use of a higher concentration of calcium in the diet (anova p-value<2e-16). The use of VitD in the diet dies not cause any significant difference in FCR (anova p-value= 0.178). Even at the recommended level of calcium (MediumLow) there is no changes in FCR between the use or no use of VitD in the diet (Fig27).

Row {data-height=500}
-----------------------------------------------------------------------
### FIGURE 27: Feed Conversion Rate days 22-33 (g:g) across treatments. <br/> There are no statistically significant changes of FCR with the use of VitD, however, there is a tendency to increase with increasing calcium levels. {data-width=500}

```{r, include=FALSE}
#FCR by alpha
#anova
#summary(aov(FCR_corr_days22_33 ~ Alphalevel,data=complete_sample_table)) #alpha not significant

#FCR by treatment
#anova
summary(aov(FCR_corr_days22_33 ~ Treatment,data=complete_sample_table)) #treatment super significant
##anova-tukey test
res.aov <- aov(FCR_corr_days22_33 ~ Treatment,data=complete_sample_table)
summary(res.aov)
tukey <- TukeyHSD(res.aov)
print(tukey)
#letters to add statistics to graph 
...
#plots with letters for statistics
plot_FCR <- ggplot(complete_sample_table, aes(x =reorder(Treatment, KitID), y = FCR_corr_days22_33, fill = Treatment)) +
  geom_boxplot(outlier.shape = NA) + #geom_boxplot()+  #or geom_violin()
  xlab("") + 
  theme(legend.position = "none") +
  geom_jitter(color="black", size=0.4, alpha=0.9) +
  #ggtitle("Feed Conversion Rate days 22-33 (g:g)") +
  labs(y = "FCR") + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  geom_text(data = cld2, aes(x = Treatment, y = 2.7, label = sig), size = 4) 
```
...
Row {data-height=90}
-----------------------------------------------------------------------
When analyzing histopathology results, the specific trait of inflammation severity in the ileum stood out. Anova test indicates inflammation severity tends to change with treatment in ileum (p-value=0.03). Specifically there is a significant change between samples of MediumLowCa and samples of MediumLowCa+VitD, the last one having lower Inflammation scores, which could indicate that the addition of VitD using the recommended level of calcium in diet can reduce the damages in the architecture or integrity of the intestines related with inflammation (Fig 28).

Row {data-height=50}
-----------------------------------------------------------------------
Hence, a t-test was done across the samples of these two groups to zoom into the previously seen difference of inflammation severity with the use of VitD, confirming it once again (t-test p-value = 0.001709) (Fig 29).


Row {data-height=600}
-----------------------------------------------------------------------
### FIGURE 28: Inflammation severity in the ileum. <br/> Impact of treatment on inflammation severity.
```{r INFLAMMATION SEVERITY ILEUM by treatment, include=FALSE}
histo <- complete_sample_table
...
print(tukey)
#letters to add statistics to graph 
cld <- multcompLetters4(res.aov, tukey, threshold = 0.1 )
cld3 <- data.frame(letters = cld$'Treatment'$Letters) #table with letters per treatment
cld3$Treatment <- rownames(cld3)
names(cld3)[1] <- "InflammationSeverity"

#PLOT INFLAMMATION ILEUM
histo_ileum2 <- histo_ileum %>% 
  pivot_longer(cols = c( "InflammationSeverity"), names_to = "InfammationSeverity", values_to = "Value")


#ileum
histo_i <- split(histo_ileum2, histo_ileum2$Treatment)
dff <- lapply(histo_i, function(x) ddply(x, .(InfammationSeverity), 
                                         summarize, mean=mean(Value)))
listDF <- list()
for (i in 1:length(dff)){
  #print(i)  
...
InfammationSeverity_plot <- ggplot(dfff, aes(y =mean , x = Treatment)) + 
  ylab("Mean Ileum Inflammation Severity") + xlab("") + 
  geom_bar(stat = "identity", fill = '#FFA076')  +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  geom_text(data = cld3, aes(x = Treatment, y = 2.5, label = InflammationSeverity)) #significance letters 


```
```{r plot INFLAMMATION SEVERITY ILEUM by treatment, include=TRUE, echo=FALSE}
InfammationSeverity_plot
```

### FIGURE 29: Ileum Inflammation severity scores for individual samples at recomended level of calcium. 
```{r inflammation by vit level, include=FALSE}
#by sample
histo_ileum3<- histo_ileum2[histo_ileum2$Treatment  %in% c("MediumLowCa+VitD","MediumLowCa"),]
t.test(Value ~ Treatment, data = histo_ileum3)

histo_by_samples_ileum <- ggplot(histo_ileum3, aes(x = Treatment, y =Value, fill=SampleID)) + 
  geom_bar(stat="identity", position="dodge", show.legend = FALSE)  +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 


```
```{r inflammation plot by vit level, include=TRUE, echo=FALSE}
histo_by_samples_ileum
```
...
Row {data-height=90}
-----------------------------------------------------------------------
Also the expression of the anti-inflammatory IL10 gene is something to look at. For the recommended level of calcium the expression of the gene changed notoriously with and without the addition of VitD in both the ileum and cecum tissues (Fig30). In the cecum the expression of the gene IL10 was increased with the use of VitD in the diet while in the ileum the behavior was contrary, and the expression decreased with the use of VitD. This may be due to a tissue specific action of the vitamin, which is expected to have a stronger effect on the ileum. 

Row {data-height=90}
-----------------------------------------------------------------------
A low expression of an anti-inflammatory gene in the ileum could indicate that the tissue is in a pro-inflammatory stage and the gene is not yet expressed to start suppressing the inflammation, which is probably not the case since the histopathology scores evidenced that there wasn't a strong inflammation of the tissue. Nevertheless, a low expression of an anti-inflammatory gene could also mean that the tissue is healthy, with no severe inflammation, hence there is no need for the anti-inflammatory gene to express. This could be better studied in an experiment with critical levels of intestinal health, in this experiment the basal levels of intestinal tissue health, especially in the ileum, are very good.

Row {data-height=500}
-----------------------------------------------------------------------
### FIGURE 30: IL10 Gene expression for treatments at recomended level of calcium. 
```{r ge il10 reco ca, include=FALSE}
GENE$TreatmentNumber = as.factor(GENE$TreatmentNumber)
GENE$SampleLocation = as.factor(GENE$SampleLocation)

#Subset kits de interés y separar por location 
subset <- GENE[GENE$TreatmentNumber %in% c(3,4),]
cec <- subset[subset$SampleLocation == "C",]
il <- subset[subset$SampleLocation == "I",]

t.test(DeltaCq_IL10 ~ Treatment, data = cec) #p-value = 0.01829
t.test(DeltaCq_IL10 ~ Treatment, data = il) #p-value = 0.0006908

#Calcular referencias (en este caso uso el tratamiento 3-MEDIUMLOWCA como mi control para el delta delta)
ref_value_IL10_i <- mean(il[il$TreatmentNumber == "3",]$DeltaCq_IL10)
ref_value_IL10_c <- mean(cec[cec$TreatmentNumber == "3",]$DeltaCq_IL10)

#calculations deltadelta and -deltadelta. 
#we plot the negative deltadelta to have a direct interpretation in the graph: 
#because the higher the deltadelta, the more cicles needed to detect the molecule, hence the lower the expresion. 
#If we plot the negative deltadelta we can interpret the plot more directly saying the higher the values in the plot, the higher the expression of the gene in the sample.  
...
summary(res.aov2) #pval= 0.017 *
summary(res.aov5) #pval= 0.000 ***
...
#plot
meanscec <- aggregate(neg_DD_IL10 ~  Treatment, cec, mean)
plotcec <- ggplot(cec, aes(x =Treatment, y = neg_DD_IL10, fill = Treatment)) +
  geom_violin() + #geom_boxplot()+  #or geom_violin()
  xlab("") + 
...
meansile <- aggregate(neg_DD_IL10 ~  Treatment, il, mean)
plotile <- ggplot(il, aes(x = Treatment, y = neg_DD_IL10, fill = Treatment)) +
  geom_violin() + #geom_boxplot()+  #or geom_violin()
  xlab("") + 
  ylab("Relative gene expression ileum") +  geom_hline(yintercept=0, linetype="dashed") + 
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  stat_summary(fun=mean, geom="point", size=2, color="red", fill="red")+ 
  geom_text(data = meansile, aes(label = round(neg_DD_IL10,2), y = neg_DD_IL10 + 0.2), 
...
```
```{r plot ge il10 reco ca, include=TRUE, echo=FALSE}
ggarrange(plotcec,plotile, nrow = 1, ncol = 2)
```
...
Row {data-height=80}
-----------------------------------------------------------------------
The SIWA ratios seek to broadly explain the composition of the microbiome in a single indicator, thus, their behavior across treatments and across vitamin D levels is of interest. Some significant differences were found as seen below, nevertheless no trend was constant, specially with the use of VitD product, which means that the AlphaD3 product does not greatly modulate gut microbiome (Fig 31 and 32).

Row {data-height=900}
-----------------------------------------------------------------------
### FIGURE 31: SIWA ratios acorss treatments {data-width=800}
```{r, include=FALSE}
ratios <- complete_sample_table
ratios$TreatmentNumber<-as.factor(ratios$TreatmentNumber)

#####Ratio 1
stat.test <- ratios %>%
  group_by(SampleLocation) %>%
  t_test(ratio1LOG ~ TreatmentNumber) %>%
  adjust_pvalue(method = "none") %>%
  add_significance(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 0.1, 1),
                   symbols = c("*****", "****", "***", "**", "*", "ns"))
#Organice to Plot
ratio1<- ggplot(ratios, aes(y =ratio1LOG , x =TreatmentNumber)) + 
  ylab("Ratio1- Lactobacillus/Escherichia-Shigella") + xlab("") + 
  geom_boxplot(fill = '#B3FF9D', outlier.shape = NA)  +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(text=element_text(size=7))+
  geom_jitter(size=0.5)+
  facet_grid(rows="SampleLocation", scales="free")+
  scale_x_discrete(labels=c("LowCa", "LowCa+VitD","MediumLowCa","MediumLowCa+VitD","MediumHighCa", "MediumHighCa+VitD","HighCa","HighCa+VitD"))
# Make facet and add p-values
stat.test <- stat.test %>% add_xy_position(x = "TreatmentNumber")
ratio1<-ratio1 +  stat_pvalue_manual(stat.test, bracket.nudge.y = -2, hide.ns = TRUE,
                                     label = "{p.adj}{p.adj.signif}", step.increase = 0.015, label.size = 2) 

#####Ratio 2
stat.test <- ratios %>%
  group_by(SampleLocation) %>%
  t_test(ratio2LOG ~ TreatmentNumber) %>%
  adjust_pvalue(method = "none") %>%
  add_significance(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 0.1, 1),
                   symbols = c("*****", "****", "***", "**", "*", "ns"))
#Organice to Plot
ratio2<- ggplot(ratios, aes(y =ratio2LOG , x =TreatmentNumber)) + 
  ylab("Ratio2- Firmicutes/ Proteobacteria") + xlab("") + 
  geom_boxplot(fill = '#9DFFD2', outlier.shape = NA)  +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(text=element_text(size=7))+
  geom_jitter(size=0.5)+
  facet_grid(rows="SampleLocation", scales="free")+
...
# Make facet and add p-values
stat.test <- stat.test %>% add_xy_position(x = "TreatmentNumber")
ratio2<-ratio2 +  stat_pvalue_manual(stat.test, bracket.nudge.y = -2, hide.ns = TRUE,
                                 label = "{p.adj}{p.adj.signif}", step.increase = 0.015, label.size = 2)
#####Ratio 3
stat.test <- ratios %>%
  group_by(SampleLocation) %>%
  t_test(ratio3LOG ~ TreatmentNumber) %>%
  adjust_pvalue(method = "none") %>%
  add_significance(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 0.1, 1),
                   symbols = c("*****", "****", "***", "**", "*", "ns"))
#Organice to Plot
ratio3<- ggplot(ratios, aes(y =ratio3LOG , x =TreatmentNumber)) + 
  ylab("Ratio3- Lactobacillus/rest of the genera") + xlab("") + 
  geom_boxplot(fill = '#9DDCFF', outlier.shape = NA)  +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(text=element_text(size=7))+
  geom_jitter(size=0.5)+
  facet_grid(rows="SampleLocation", scales="free")+
...
# Make facet and add p-values
stat.test <- stat.test %>% add_xy_position(x = "TreatmentNumber")
ratio3<-ratio3 +  stat_pvalue_manual(stat.test, bracket.nudge.y = -2, hide.ns = TRUE,
                                     label = "{p.adj}{p.adj.signif}", step.increase = 0.015, label.size = 2) 

```
```{r, include=TRUE, echo=FALSE}

ggarrange(ratio1, ratio2, ratio3, ncol=3, nrow = 1)

```

### FIGURE 32: SIWA ratios with and without vitamin D 
```{r, include=FALSE}
#####Ratio 1
stat.test <- ratios %>%
  group_by(SampleLocation) %>%
  t_test(ratio1LOG ~ Alphad3_level) %>%
  adjust_pvalue(method = "none") %>%
  add_significance(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 0.1, 1),
                   symbols = c("*****", "****", "***", "**", "*", "ns"))
#Organice to Plot
ratio1<- ggplot(ratios, aes(y =ratio1LOG , x =Alphad3_level)) + 
  ylab("Ratio1- Lactobacillus/Escherichia-Shigella") + xlab("") + 
  geom_boxplot(fill = '#B3FF9D', outlier.shape = NA)  +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  geom_jitter(size=0.5)+
...
# Make facet and add p-values
stat.test <- stat.test %>% add_xy_position(x = "Alphad3_level")
ratio1<-ratio1 +  stat_pvalue_manual(stat.test, bracket.nudge.y = -2, hide.ns = TRUE,
                                     label = "{p.adj}{p.adj.signif}", step.increase = 0.015)


#####Ratio 2
stat.test <- ratios %>%
  group_by(SampleLocation) %>%
  t_test(ratio2LOG ~ Alphad3_level) %>%
  adjust_pvalue(method = "none") %>%
  add_significance(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 0.1, 1),
                   symbols = c("*****", "****", "***", "**", "*", "ns"))
#Organice to Plot
ratio2<- ggplot(ratios, aes(y =ratio2LOG , x =Alphad3_level)) + 
  ylab("Ratio2- Firmicutes/ Proteobacteria") + xlab("") + 
  geom_boxplot(fill = '#9DFFD2', outlier.shape = NA)  +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  geom_jitter(size=0.5)+
...
stat.test <- ratios %>%
  group_by(SampleLocation) %>%
  t_test(ratio3LOG ~ Alphad3_level) %>%
  adjust_pvalue(method = "none") %>%
  add_significance(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 0.1, 1),
                   symbols = c("*****", "****", "***", "**", "*", "ns"))
#Organice to Plot
ratio3<- ggplot(ratios, aes(y =ratio3LOG , x =Alphad3_level)) + 
  ylab("Ratio3- Lactobacillus/rest of the genera") + xlab("") + 
  geom_boxplot(fill = '#9DDCFF', outlier.shape = NA)  +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  geom_jitter(size=0.5)+
...
Row {data-height=100}
-----------------------------------------------------------------------
In this case we evaluated how the different calcium levels fall into a larger population of 42 days old broilers in terms of SIWA ratio 1 (Lactobacillus/ Escherichia-Shigella) (Fig 33). As well as with and without vitamin D with the recomended level of calclium (Fig 34). In none of the benchmarks the groups of interest stood out of the average range for SIWA Ratio 1. The same was found using alpha diversity of the microbiome as an indicator (not shown). Once again indicating that either (calcium level or vitamin use) modulate greatly the microbiome composition.


Row {data-height=400}
-----------------------------------------------------------------------
### FIGURE 33: Benchmark of different levels of calcium un diet{data-with=500}
```{r picture bench all, include= TRUE, echo=FALSE, out.width = '100%'}
knitr::include_graphics("Benchmark_alltreatments.jpeg")
```

### FIGURE 34: Benchmark of recomended level of calcium with and without vitamin{data-with=500}
```{r picture bench 3 and 4, include= TRUE, echo=FALSE, out.width = '100%'}
knitr::include_graphics("Benchmark_treatments3and4.jpeg")
```
...
Row {data-height=70}
-----------------------------------------------------------------------
Differential abundance analyzes were performed to understand which bacteria are more abundant in one group compared to another. Then comparing the recommended level of calcium without the use of vitamin D vs. the same level using vitamin D some differing bacteria were found (Fig 35A). However, it must be taken into account that these results are very specific for this calcium level, when doing so for the other calcium levels the results were not consistent and different bacteria appeared differentially abundant (not shown).

```{r dif abund vit, include=FALSE}
### leer archivo ya generado
list_dfs_filtered <- readRDS("/Volumes/GoogleDrive/Mi unidad/SIWAproject/Reports/Projects/E335_MS/diff_abund/list_dfs_filtered_bygenus.RData")
groups=list_dfs_filtered$groups
list_dfs_filtered$groups[[227]]
```

Row {data-height=500}
-----------------------------------------------------------------------
### FIGURE 35A: Differential abundance between presenece and absense of VitD with recomended level of calcium- cecum {data-with=500}
```{r dif abund vit cecum, include=TRUE, echo=FALSE}
plot_deseq_report_with_color_easyread_for_genera (list_dfs_filtered, 71, "Cecum: MediumLowCa(3) vs MediumLowCa+VitD(4)", species_taxonomy_info, genera_taxonomy_info,  size=8)
```

### FIGURE 35B: Differential abundance between presenece and absense of VitD with recomended level of calcium- ileum. {data-with=500}
```{r dif abund vit ile, include=TRUE, echo=FALSE}
plot_deseq_report_with_color_easyread_for_genera (list_dfs_filtered, 227, "Ileum: MediumLowCa(3) vs MediumLowCa+VitD(4)", species_taxonomy_info, genera_taxonomy_info,  size=8)
```
...
Row {data-height=100}
-----------------------------------------------------------------------
In addition, a partition of samples was made not by their diet but by their performance and it was explored if there was any differentially abundant bacteria between the samples that had a good performance (1 lower FCR) vs. the samples that had low performance (2 higher FCR) (Fig 36). Indeed there are some bacterial groups that differ between these conditions of the animal, however, they do not match the groups that are increased when using vitamin D in the diet. Therefore it is not possible to affirm that the vitamin D increases bacteria that are beneficial for the animal or are related to an increase in productivity.


Row {data-height=500}
-----------------------------------------------------------------------
### FIGURE 36A: Differential abundance between high and low perfomance animals- cecum. {data-with=500}
...
### FIGURE 36B: Differential abundance between high and low perfomance animals- cecum. {data-with=500}
```{r, include=TRUE, echo=FALSE, warning=FALSE}
...
Row {data-height=100}
-----------------------------------------------------------------------
In terms of performance, feed conversion rate increases with the increase of calcium concentration in the diet and the use of vitamin D does not seem to relieve the effect. However, at the recommended level of calcium (MedLow) the use of vitamin D has correlation with benefits such as the improvement of mucosal integrity and overall gut architecture, the reduction of inflammation mainly of the ileum tissue, changes in expression of anti-inflamatory genes. 

Row {data-height=150}
-----------------------------------------------------------------------
The microbiome, only with extreme low or extreme high calcium concentrations evidences changes in microbiome composition in terms of beta divesity. But within a regular range of calcium concentration neither calcium level or the use of vitamin D seem to have a great and consistent effect on its modulation, which could be considered as beneficial as it does not interfere greatly in order to cause a dysbiosis. 