...
#Create genus label for plot including "Genera < 1% abund."
grouped$meanrelativeabunance = rowMeans(grouped)
grouped$genus <- row.names(grouped) 
grouped$genus_label <- "na"
grouped <- grouped %>% mutate(genus_label = if_else(meanrelativeabunance > 0.01, genus, "Genera < 1% abund."))
genus_labels <- grouped$genus_label
names(genus_labels) <- rownames(grouped)
melted$label <- as.character(lapply(melted$OTU, function(x){genus_labels[x]}))
#add treatment info 
...
#add treatment info 
melted$SampleID <- (melted$Sample)
metadata_cec <- sample_data(ODLEPobj_ileum)
melted <- dplyr::left_join(melted, metadata_cec, by="SampleID")
melted <- melted %>%
  mutate( Treatment=factor(Treatment,levels=c("LowCa", "LowCa+VitD", "MediumLowCa", "MediumLowCa+VitD", "MediumHighCa", "MediumHighCa+VitD", "HighCa","HighCa+VitD")) )
...
#add treatment info 
melted$SampleID <- (melted$Sample)
metadata_cec <- sample_data(ODLEPobj_feces)
melted <- dplyr::left_join(melted, metadata_cec, by="SampleID")
melted <- melted %>%
  mutate( Treatment=factor(Treatment,levels=c("LowCa", "LowCa+VitD", "MediumLowCa", "MediumLowCa+VitD", "MediumHighCa", "MediumHighCa+VitD", "HighCa","HighCa+VitD")) )
