...
### TABLE 1: Samples per group {data-width=800}
...
subset.genus.df <- subset.genus.df %>%
  mutate( Treatment=factor(Treatment,levels=c("LowCa", "LowCa+MediumVitD", "MediumLowCa", "MediumLowCa+MediumVitD", "MediumHighCa", "MediumHighCa+MediumVitD", "HighCa","HighCa+MediumVitD")) )
...
Row {data-height=500}
-----------------------------------------------------------------------
### FIGURE 4A: Taxonomic composition CECUM {data-width=1000}
...
Row {data-height=500}
-----------------------------------------------------------------------
### FIGURE 4B: Taxonomic composition ILEUM {data-width=1000}
```{r, echo=FALSE, include=TRUE}
...
Row {data-height=500}
-----------------------------------------------------------------------
### FIGURE 4C: Taxonomic composition FECES {data-width=1000}
```{r, echo=FALSE, include=TRUE}
...
Row {data-height=500}
-----------------------------------------------------------------------
### FIGURE 7:Dissimilarity between all samples (Bray distances) {data-width=1000}
```{r , echo=FALSE}
plot.beta.bray
```
...
Row {data-height=100}
-----------------------------------------------------------------------
After comparing the grouping of samples by composition distances evidencing general trends in composition between treatments, we went on to look at methods to identify more specific changes. Microbiome data can be filtered, merged, and subset to make specific comparisons between any two groups, and at any taxonomic level. This with the goal to identify features (i.e., species, OTUs, gene families, etc.) that differ in abundance between two groups of samples according to their treatment. (We here use <a href="https://genomebiology.biomedcentral.com/articles/10.1186/gb-2010-11-10-r106" target="_blank">DESeq</a> method).
...
groupnamesST <- readRDS(file=paste0(folder, "groupnamesST.RData"))

plot_deseq_congress <-
  function(dfs_filtered, comparisonINDEX, Title) {
    theme_set(theme_bw())
    ...
    plot = ggplot(df, aes(x = log2FoldChange, y = taxa, color = taxa)) + geom_point(size =8) + labs(fill = "taxa", color = "taxa")  +
      theme(legend.position = "none") +
      ggtitle(Title)
    return(plot)
  }
...
### FIGURE 11A: Cecum diff - VitD level  {data-width=500}

```{r , echo=FALSE}
comparison= 1
#print(groupnamesST[[comparison]])
Title= "Cecum 0 VitD vs 1 VitD"
p_cecum = plot_deseq_congress(dfs_filtered, comparison, Title)  
p_cecum  = p_cecum + theme(axis.title.x=element_blank())
p_cecum
```
...
p_feces = plot_deseq_congress(dfs_filtered, comparison, Title)  
p_feces
```
...
histo <- histo %>% 
  mutate(Treatment = if_else(KitID == 70, "LowCa", 
                                ifelse(KitID == 71, "LowCa+MediumVitD",
                                       ifelse(KitID == 72, "MediumLowCa",
                                              ifelse(KitID == 73, "MediumLowCa+MediumVitD",
                                                     ifelse(KitID == 74, "MediumHighCa",
                                                            ifelse(KitID == 75, "MediumHighCa+MediumVitD",
                                                                   ifelse(KitID == 76, "HighCa",
                                                                          ifelse(KitID == 77, "HighCa+MediumVitD","other")))))))))
...
### FIGURE 12: Histopathology scores agreggated by Treatment - Cecum 
...
### FIGURE 13: FIGURE 12: Histopathology scores agreggated by Treatment - Ileum
                             ...
                             ifelse(KitID == 71, "LowCa+MediumVitD",
                                    ifelse(KitID == 72, "MediumLowCa",
                                           ifelse(KitID == 73, "MediumLowCa+MediumVitD",
                                                  ifelse(KitID == 74, "MediumHighCa",
                                                         ifelse(KitID == 75, "MediumHighCa+MediumVitD",
                                                                ifelse(KitID == 76, "HighCa","HighCa+MediumVitD"))))))))
...
plot1 <- ggplot(il, aes(x = Treatment, y = RQ_MUC2_log, fill = Treatment)) +
  ...
  geom_text(data = IL10_ile, aes(x = Treatment, y = 5, label = IL10_ile), size = 5)
plot1
```
...
```{r TIGNORE TEST, include=FALSE}

#subset.genus.df_cecum[is.na(subset.genus.df_cecum$OverallArchitectureLEVEL),]
#histo[histo$SampleID == "0071_05C-M	"]
#subset.genus.df_cecum[!is.na(subset.genus.df_cecum[i]),]
#hacer grid de graficas CECUM

x <- subset.genus.df_cecum[!is.na(subset.genus.df_cecum[c]),]  %>% dplyr::select(OTU,Abundance,
...
ggplotly(p) %>% 
 ...
 legend = list(orientation="h", y=-0.2, title = list(text = "Genus"))) %>% partial_bundle()
```

Row {data-height=500}
-----------------------------------------------------------------------
### TABLE 4: Cecum- Sample count per category in each score {data-width=200}

```{r imprimir cecum tablas con conteo de muestras, include=TRUE, echo=FALSE, warning=FALSE, fig.width = 10}
options(knitr.kable.NA = 0)
kbl(t(table_sample_count_cecum), row.names = TRUE) %>% kable_styling(position= "center", full_width = F)
```

### FIGURE 17A {data-width=400}
```{r 17a, include=TRUE, echo=FALSE, warning=FALSE}
c <- "OverallArchitectureLEVEL"
p <- ggplot(data=subset.genus.df_cecum[!is.na(subset.genus.df_cecum[c]),], 
              aes_string(x=c, y="Abundance",fill="OTU")) + 
    geom_bar(stat="identity", position="fill") + 
    xlab(categories_labels[c]) +
    theme(legend.position = "bottom") +
    theme(axis.text.x = element_text(angle = 60, hjust = 1))
ggplotly(p) %>% 
 layout( 
...
### FIGURE 17B {data-width=400}
```{r 17B, include=TRUE, echo=FALSE, warning=FALSE}

c <- "MucosalIntegrityLEVEL"
p <- ggplot(data=subset.genus.df_cecum[!is.na(subset.genus.df_cecum[c]),], 
              aes_string(x=c, y="Abundance",fill="OTU")) + 
    geom_bar(stat="identity", position="fill") + 
    xlab(categories_labels[c]) +
...
### FIGURE 17C
```{r 17C, include=TRUE, echo=FALSE, warning=FALSE}
c = "LymphoidImmuneLEVEL"
p <- ggplot(data=subset.genus.df_cecum[!is.na(subset.genus.df_cecum[c]),], 
              aes_string(x=c, y="Abundance",fill="OTU")) + 
    geom_bar(stat="identity", position="fill") + 
    xlab(categories_labels[c]) +
...
```
### FIGURE 17D
```{r 17D, include=TRUE, echo=FALSE, warning=FALSE}
c = "InflammationSeverityLEVEL"
p <- ggplot(data=subset.genus.df_cecum[!is.na(subset.genus.df_cecum[c]),], 
              aes_string(x=c, y="Abundance",fill="OTU")) + 
    geom_bar(stat="identity", position="fill") + 
    xlab(categories_labels[c]) +
...
```
### FIGURE 17E
```{r 17E, include=TRUE, echo=FALSE, warning=FALSE}
c = "MicrobialOrganismsLEVEL"
p <- ggplot(data=subset.genus.df_cecum[!is.na(subset.genus.df_cecum[c]),], 
              aes_string(x=c, y="Abundance",fill="OTU")) + 
    geom_bar(stat="identity", position="fill") + 
    xlab(categories_labels[c]) +
...
kbl(t(table_sample_count_ileum), row.names = TRUE) %>% kable_styling(position= "left", full_width = T)

```


### FIGURE 18A
```{r 18a, include=TRUE, echo=FALSE, warning=FALSE}

c = "OverallArchitectureLEVEL"
p <- ggplot(data=subset.genus.df_ileum[!is.na(subset.genus.df_ileum[c]),], 
              aes_string(x=c, y="Abundance",fill="OTU")) + 
    geom_bar(stat="identity", position="fill") + 
    xlab(categories_labels[c]) +
    ylab(NULL)+
...
### FIGURE 18B
```{r 18B, include=TRUE, echo=FALSE, warning=FALSE}
...
### FIGURE 18C
```{r 18C, include=TRUE, echo=FALSE, warning=FALSE}
...
```
### FIGURE 18D
```{r 18D, include=TRUE, echo=FALSE, warning=FALSE}
...
```
### FIGURE 18E
```{r 18E, include=TRUE, echo=FALSE, warning=FALSE}
...
Row 
-----------------------------------------------------------------------
### FIGURE 21: Alpha diversity vs Gene expression - Cecum {data-width=500}
...
Row 
-----------------------------------------------------------------------
### FIGURE 22: Alpha diversity vs Gene expression - Ileum {data-width=500}
...
Row 
-----------------------------------------------------------------------
### FIGURE 23: Ratios vs Gene Expression - Cecum  {data-width=500}
...
Row
-----------------------------------------------------------------------
### FIGURE 24: Ratios vs Gene Expression - Ileum {data-width=500}
...
### FIGURE 25A: Histo vs Alpha diversity - Cecum {data-width=500}
...
### FIGURE 26A: Histo vs Ratios - Cecum {data-width=500}
